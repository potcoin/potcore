...
var potcoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in potcoreDefaults)
    if (potcoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = potcoreDefaults[i];
